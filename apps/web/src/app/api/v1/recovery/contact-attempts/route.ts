...
import { createContactAttemptUseCase, RECOVERY_PERMISSIONS } from "@/features/recovery-core/server";
...
export const POST = withApiRoute(async (r, c) => {
  ...
  return apiCreated(
    await createContactAttemptUseCase(await readJsonBody(r), { actor, correlationId: c }),
    c,
  );
});
