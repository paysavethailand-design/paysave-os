...
import type { SecurityReviewCheck } from "../domain/security-review";
import { SecurityReviewStatusBadge } from "./security-review-status-badge";

export function SecurityReviewSection({
  id,
  title,
  description,
  checks,
          ...
          <Card key={check.id}>
            <CardHeader className="space-y-3">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <SecurityReviewStatusBadge status={check.status} />
              </div>
                ...
                {check.evidence.length > 0 ? (
                  <ul className="mt-2 flex flex-wrap gap-2">
                    {check.evidence.map((item) => (
                      <li key={item}>
                        <Badge variant="neutral" className="max-w-full break-all">
                          {item}
                        </Badge>
                      </li>
                    ))}
