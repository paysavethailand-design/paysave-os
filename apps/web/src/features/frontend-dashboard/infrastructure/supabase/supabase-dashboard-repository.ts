...
export class SupabaseDashboardRepository implements DashboardRepository {
  ...
  async getDashboard(persona: DashboardPersona): Promise<DashboardModel> {
    ...
    if (ctx) {
      ...
      if (!hasAccess) {
        ...
        return {
          ...
          description:
            "Your role does not permit this dashboard view. RLS policies also enforce at DB level.",
          kpis: [],
          trend: [],
          distribution: [],
          activity: [],
    ...
    const baseKpis: readonly DashboardKpi[] = this.computeKpis(
      persona,
      totalCases,
      openCases,
      totalAssets,
  ...
  private computeKpis(
    ...
    const common = [
      [
        "มูลค่าพอร์ต",
        `฿${fmt(Math.max(50, totalAssets) * 1200000)}`,
        "+8.4%",
        "up",
        "เทียบเดือนก่อน",
      ] as const,
      [
        "รับชำระเดือนนี้",
        `฿${fmt(Math.max(10, Math.floor(totalCases / 5)) * 12000)}`,
        "+12.4%",
        "up",
        "เป้าหมาย 78%",
      ] as const,
      [
        "อัตราสำเร็จ",
        `${Math.min(95, Math.max(55, 70 + Math.floor(openCases / 10)))}%`,
        "+4.1%",
        "up",
        "สูงกว่า SLA",
      ] as const,
      [
        "รายการเสี่ยง",
        fmt(Math.max(5, Math.floor(totalCases * 0.3))),
        "-9.8%",
        "up",
        "ลดลง 6 รายการ",
      ] as const,
    ...
    switch (persona) {
      case "executive":
        return common.map(([label, value, trendValue, direction, helper]) => ({
          label,
          value,
          trend: trendValue,
          direction,
      ...
      case "partner":
        return [
          {
            label: "สัญญาที่ดูแล",
            value: fmt(Math.max(100, totalCases)),
            trend: "+6.2%",
            direction: "up",
          ...
          {
            label: "ยอดครบกำหนด",
            value: `฿${fmt(Math.max(5, openCases) * 180000)}`,
            trend: "+2.8%",
            direction: "neutral",
            helper: "รอบเดือนนี้",
          },
          {
            label: "รับชำระแล้ว",
            value: `฿${fmt(Math.max(3, totalCases) * 150000)}`,
            trend: "+10.1%",
            direction: "up",
            helper: "82.1% ของกำหนด",
          },
          {
            label: "Promise to Pay",
            value: fmt(Math.max(20, Math.floor(openCases * 0.7))),
            trend: "-3.2%",
            direction: "down",
            helper: "ต้องติดตาม 41",
          },
        ];
      case "admin":
        return [
          {
            label: "ผู้ใช้ทั้งหมด",
            value: fmt(Math.max(50, totalAssets + 200)),
            trend: "+28",
            direction: "up",
            helper: "เดือนนี้",
          },
          {
            ...
            value: fmt(Math.max(30, Math.floor(totalCases * 0.8))),
            trend: "+4.0%",
            direction: "neutral",
            helper: "12 Partner",
          },
          ...
          {
            label: "Failed Login",
            value: fmt(Math.max(1, Math.floor(openCases / 10))),
            trend: "-41%",
            direction: "up",
            helper: "24 ชั่วโมง",
          },
      ...
      case "field":
        return [
          {
            label: "งานวันนี้",
            value: fmt(Math.max(5, Math.floor(openCases / 4))),
            trend: "+3",
            direction: "neutral",
            helper: "จากเมื่อวาน",
          },
          {
            label: "เสร็จแล้ว",
            value: `${Math.floor(Math.min(95, 60 + openCases / 5))}%`,
            trend: "61%",
            direction: "up",
            helper: "ตามแผน",
          },
          ...
          {
            label: "ระยะทาง",
            value: `${Math.max(10, Math.floor(totalCases / 5))} km`,
            trend: "-8 km",
            direction: "up",
            helper: "เส้นทางเหมาะสม",
          },
        ];
      default:
        return common.map(([label, value, trendValue, direction, helper]) => ({
